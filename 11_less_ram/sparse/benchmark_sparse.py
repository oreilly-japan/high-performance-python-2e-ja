...
def benchmark(size=2048):
    ...
    for density in densities:
        ...
        N_sparse, total_time_sparse = timeit.Timer("m_sparse * m_sparse", globals=locals()).autorange()
        N_dense, total_time_dense = timeit.Timer("m_dense * m_dense", globals=locals()).autorange()
    ...
    fig = py.figure()
    py.plot(densities * 100, results["sparse"], "--", marker="s", label="CSR Sparse Matrix")
...
def matrix_size(size=2028):
    ...
    for density in densities:
        ...
        sparse_size.append((m_sparse.data.nbytes + m_sparse.indices.nbytes + m_sparse.indptr.nbytes) / 1e6)
